...
$script:AppVersion = "2.7.6"
$script:BuildDate  = "2026-02-04"
...
.DESCRIPTION
    ...
    Uses password-based encryption with integrity (AES-GCM when available; otherwise AES-256-CBC + HMAC-SHA256) and PBKDF2 key derivation.
    Features:
                    ...
                    <StackPanel Margin="10">
                        ...
                        <Border Background="White" BorderBrush="#BDC3C7" 
                            ...
                            <StackPanel>
                                ...
                                <TextBlock Text="Profiles" FontSize="14" FontWeight="Bold" Margin="0,0,0,6"/>
                                <StackPanel Orientation="Horizontal" Margin="0,0,0,6">
                                    <TextBlock Text="Profile:" VerticalAlignment="Center" Foreground="#2C3E50"/>
                                    <ComboBox Name="cmbProfiles" IsEditable="True" Width="220" Height="28" Margin="10,0,0,0"/>
                                    <Button Name="btnLoadProfile" Content="Load" Width="70" Margin="10,0,0,0"/>
                                    <Button Name="btnSaveProfile" Content="Save" Width="70" Margin="6,0,0,0"/>
                                    <Button Name="btnDeleteProfile" Content="Delete" Width="70" Margin="6,0,0,0"/>
                                </StackPanel>
                                <TextBlock Name="txtProfileHint" Text="Profiles save settings (cloud paths, providers, sync options, watch folders). Password is not stored." Foreground="#7F8C8D" FontSize="11" TextWrapping="Wrap"/>

                                <!-- Watch Folders -->
                                <Separator Margin="0,12,0,12"/>
                                <TextBlock Text="Watch Folders" FontSize="14" FontWeight="Bold" Margin="0,0,0,6"/>
                                <CheckBox Name="chkWatchEnabled" Content="Monitor folders and auto-encrypt new files" Margin="0,0,0,6"/>
                                <ListBox Name="lstWatchFolders" Height="90" Margin="0,0,0,6"/>
                                <StackPanel Orientation="Horizontal">
                                    <Button Name="btnAddWatchFolder" Content="Add Folder" Width="90"/>
                                    <Button Name="btnRemoveWatchFolder" Content="Remove" Width="90" Margin="6,0,0,0"/>
                                    <Button Name="btnStartWatching" Content="Start" Width="70" Margin="14,0,0,0"/>
                                    <Button Name="btnStopWatching" Content="Stop" Width="70" Margin="6,0,0,0"/>
                                </StackPanel>
</StackPanel>
                        </Border>
    ...
    <Expander Grid.Row="2" Header="Logs" Margin="0,10,0,0">
        <Grid>
            ...
            <StackPanel Grid.Row="0" Orientation="Horizontal" Margin="0,0,0,5">
                ...
                <Button Name="btnClearSyncQueue" Content="Clear Sync Queue" Width="140" Height="26" Margin="10,0,0,0"/>
...
# CFEM runtime paths
function Get-CFEMAppDataDir {
    $dir = Join-Path $env:APPDATA "CFEM"
    if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
    return $dir
}

function Get-ProfilesPath { return (Join-Path (Get-CFEMAppDataDir) "profiles.json") }
...
function Save-Profiles($profiles) {
    $path = Get-ProfilesPath
    ($profiles | ConvertTo-Json -Depth 10) | Set-Content -Path $path -Encoding UTF8
}

function Get-CurrentSettingsSnapshot {
    return @{
        OneDriveEnabled    = [bool]$script:chkOneDrive.IsChecked
        OneDrivePath       = [string]$script:txtOneDrivePath.Text
        GoogleDriveEnabled = [bool]$script:chkGoogleDrive.IsChecked
        GoogleDrivePath    = [string]$script:txtGoogleDrivePath.Text
        DropboxEnabled     = [bool]$script:chkDropbox.IsChecked
        DropboxPath        = [string]$script:txtDropboxPath.Text
        SyncToCloud        = [bool]$script:chkSyncToCloud.IsChecked
        CloudSubfolder     = [string]$script:txtCloudSubfolder.Text
        WatchEnabled       = [bool]$script:chkWatchEnabled.IsChecked
        WatchFolders       = @($script:lstWatchFolders.Items)
    }
}

function Apply-SettingsSnapshot($s) {
    if (-not $s) { return }
    $script:chkOneDrive.IsChecked     = [bool]$s.OneDriveEnabled
    $script:txtOneDrivePath.Text      = [string]$s.OneDrivePath
    $script:chkGoogleDrive.IsChecked  = [bool]$s.GoogleDriveEnabled
    $script:txtGoogleDrivePath.Text   = [string]$s.GoogleDrivePath
    $script:chkDropbox.IsChecked      = [bool]$s.DropboxEnabled
    $script:txtDropboxPath.Text       = [string]$s.DropboxPath
    $script:chkSyncToCloud.IsChecked  = [bool]$s.SyncToCloud
    if ($s.CloudSubfolder) { $script:txtCloudSubfolder.Text = [string]$s.CloudSubfolder }

    $script:chkWatchEnabled.IsChecked = [bool]$s.WatchEnabled
    $script:lstWatchFolders.Items.Clear()
    foreach ($f in @($s.WatchFolders)) {
        if ($f) { [void]$script:lstWatchFolders.Items.Add([string]$f) }
    }
}

function Refresh-ProfilesUI {
    $profiles = Load-Profiles
    $script:cmbProfiles.Items.Clear()
    foreach ($k in ($profiles.Keys | Sort-Object)) { [void]$script:cmbProfiles.Items.Add($k) }
}

# --- Watch folders (auto-encrypt newly created files) ---
$script:Watchers = @()
...
function Stop-Watching {
    ...
    $script:Watchers = @()
    Write-GuiLog "Watch: stopped."
    Update-Status -Message "Watch stopped." -Detail ""
}
...
function Start-Watching {
    if (-not $script:chkWatchEnabled.IsChecked) {
        Write-GuiLog "Watch: not enabled."
        return
    }
    if ($script:Watchers.Count -gt 0) { Stop-Watching }

    $pwd = $script:txtPassword.Password
    if ([string]::IsNullOrWhiteSpace($pwd)) {
        Write-GuiLog "Watch: password is required to start watching."
        Update-Status -Message "Watch not started - enter a password first." -Detail ""
        return
    }

    $folders = @($script:lstWatchFolders.Items)
    if (-not $folders -or $folders.Count -eq 0) {
        Write-GuiLog "Watch: no folders configured."
        Update-Status -Message "Watch not started - add at least one folder." -Detail ""
        return
    }

    foreach ($folder in $folders) {
        if (-not (Test-Path $folder)) { Write-GuiLog "Watch: missing folder: $folder"; continue }
        ...
        $action = {
            ...
            try {
                ...
                if ($full -like "*.encrypted") { return }
                if (-not (Wait-FileReady -Path $full -TimeoutSeconds 45)) { return }

                $pwdLocal = $script:txtPassword.Password
                if ([string]::IsNullOrWhiteSpace($pwdLocal)) { return }

                $jobFolder = New-EncryptionJobOutputRoot
                $sourceRoot = $sender.Path
                $out = Encrypt-FileCFEM -FilePath $full -Password $pwdLocal -OutputRoot $jobFolder -SourceRoot $sourceRoot

                Write-GuiLog "Watch: encrypted new file: $full -> $out"

                if ($script:chkSyncToCloud.IsChecked) {
                    Queue-CloudSyncForOutputs -OutputPaths @($out)
                    $sum = Process-SyncQueue
                    Write-GuiLog ("Watch sync - Copied: {0} Failed: {1} Pending: {2}" -f $sum.Copied, $sum.Failed, $sum.Pending)
                }
            } catch {
                Write-GuiLog ("Watch error: {0}" -f $_.Exception.Message)
            }
        }
        ...
        $script:Watchers += $fsw
        Write-GuiLog "Watch: started for $folder"
    }

    Update-Status -Message "Watch started." -Detail ""
}

# --- Authenticated encryption: AES-GCM when available; otherwise AES-256-CBC + HMAC-SHA256 ---
function Test-AesGcmAvailable {
    try { return [bool]([type]::GetType("System.Security.Cryptography.AesGcm")) } catch { return $false }
}
...
function Encrypt-FileAESGCM {
    ...
    Write-Log "Encrypting file (AES-GCM): $FilePath"
    ...
    $cipher = New-Object byte[] $fileBytes.Length
    $tag = New-Object byte[] 16

    $aesgcm = [System.Security.Cryptography.AesGcm]::new($key)
    $aesgcm.Encrypt($nonce, $fileBytes, $cipher, $tag, $null)

    # Output path selection
    $outDir = if ($OutputRoot) { $OutputRoot } else { Split-Path $FilePath -Parent }
    if ($OutputRoot -and $SourceRoot) {
    ...
    $outFile = Join-Path $outDir ((Split-Path $FilePath -Leaf) + $OutputExtension)

    # Format: "CFEMGCM1" (7 bytes) + salt32 + nonce12 + tag16 + ciphertext
    $magic = [System.Text.Encoding]::ASCII.GetBytes("CFEMGCM1")
    $outBytes = $magic + $salt + $nonce + $tag + $cipher
    [System.IO.File]::WriteAllBytes($outFile, $outBytes)

    return $outFile
}

function Encrypt-FileCFEM {
    param(
        [string]$FilePath,
        [string]$Password,
        [int]$Iterations = 250000,
    ...
    if (Test-AesGcmAvailable) {
        return (Encrypt-FileAESGCM -FilePath $FilePath -Password $Password -Iterations $Iterations -OutputExtension $OutputExtension -OutputRoot $OutputRoot -SourceRoot $SourceRoot)
    } else {
        return (Encrypt-FileAES -FilePath $FilePath -Password $Password -Iterations $Iterations -OutputExtension $OutputExtension -OutputRoot $OutputRoot -SourceRoot $SourceRoot)
    }
}
...
function Decrypt-FileAES {
    ...
    try {
        ...
        # CFEM AES-GCM format detection (magic "CFEMGCM1")
        if ($encryptedData.Length -ge (7 + 32 + 12 + 16 + 1)) {
            $magic = [System.Text.Encoding]::ASCII.GetString($encryptedData, 0, 7)
            if ($magic -eq "CFEMGCM1") {
                if (-not (Test-AesGcmAvailable)) { throw "AES-GCM is not available in this PowerShell/.NET runtime." }

                $offset = 7
                $salt = $encryptedData[$offset..($offset+31)]
                $offset += 32
                $nonce = $encryptedData[$offset..($offset+11)]
                $offset += 12
                $tag = $encryptedData[$offset..($offset+15)]
                $offset += 16
                $cipherText = $encryptedData[$offset..($encryptedData.Length - 1)]
                ...
                $outputPath = $FilePath
                if ($outputPath.EndsWith($InputExtension, [System.StringComparison]::OrdinalIgnoreCase)) {
                    $outputPath = $outputPath.Substring(0, $outputPath.Length - $InputExtension.Length)
                } else {
                    $outputPath = $outputPath + ".decrypted"
                }

                [System.IO.File]::WriteAllBytes($outputPath, $plain)
                Write-Log "Successfully decrypted (AES-GCM): $FilePath -> $outputPath"
                return $outputPath
            }
        }
...
function global:Write-GuiLog {
    ...
    # Build a consistent line
    $ts = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss")
    $line = "[$ts] [$Level] $Message"

    # Always attempt to write via primary logger if available
    try {
        $wl = Get-Command -Name Write-Log -ErrorAction SilentlyContinue
        if ($wl) {
            Write-Log -Message $Message -Level $Level
        } else {
            # Bootstrap fallback (so errors are never "lost" during early init or runspace actions)
            $bootstrapDir = Join-Path $env:LOCALAPPDATA "CloudFileEncryptionManager\Logs"
            if (-not (Test-Path $bootstrapDir)) { New-Item -ItemType Directory -Path $bootstrapDir -Force | Out-Null }
            $bootstrapLog = Join-Path $bootstrapDir "CFEM_bootstrap.log"
            Add-Content -Path $bootstrapLog -Value $line -Encoding UTF8
        }
    } catch {
        # Last-ditch fallback
        try { Add-Content -Path (Join-Path $env:TEMP "CFEM_bootstrap.log") -Value $line -Encoding UTF8 } catch {}
    }

    # Best-effort UI updates (do not throw if UI isn't available in this runspace)
    try {
        if ($script:txtStatus) { $script:txtStatus.Text = $Message }
        if ($script:txtProgressStatus) { $script:txtProgressStatus.Text = $Message }

        if ($script:txtLiveLog) {
            $append = {
                param($t)
                try {
                    $script:txtLiveLog.AppendText($t + [Environment]::NewLine)
                    $script:txtLiveLog.ScrollToEnd()
                } catch {}
            }

            # Prefer dispatcher invoke when available
            if ($script:txtLiveLog.Dispatcher -and -not $script:txtLiveLog.Dispatcher.CheckAccess()) {
                $script:txtLiveLog.Dispatcher.Invoke($append, $line) | Out-Null
            } else {
                & $append $line
            }
        }
...
# Make logging functions available to event/runspace callbacks
try { 
    Set-Item -Path function:global:Write-GuiLog -Value ${function:Write-GuiLog} -Force 
    Set-Item -Path function:script:Write-GuiLog -Value ${function:Write-GuiLog} -Force 
} catch {
    Write-Warning "Failed to propagate Write-GuiLog to global scope: $_"
}
...
function Clear-SyncQueue {
    # Remove queue files from all known locations (Roaming + legacy LocalAppData)
    $paths = @()
    $paths += (Join-Path $env:APPDATA "CloudFileEncryptionManager\sync-queue.json")
    $paths += (Join-Path $env:LOCALAPPDATA "CloudFileEncryptionManager\sync-queue.json")

    foreach ($p in $paths | Select-Object -Unique) {
        try {
            if (Test-Path $p) {
                Remove-Item -Path $p -Force -ErrorAction Stop
                Write-Log "Cleared sync queue file: $p" -Level Info
            }
        } catch {
            Write-Log "Failed to clear sync queue file: $p : $($_.Exception.Message)" -Level Warn
        }
    }

    # Ensure a fresh empty queue exists at the primary path so subsequent runs are deterministic
    try {
        $primary = Get-QueuePath
        @() | ConvertTo-Json -Depth 6 | Set-Content -Path $primary -Encoding UTF8
        Write-Log "Initialized empty sync queue at: $primary" -Level Info
    } catch {
        Write-Log "Failed to initialize empty sync queue: $($_.Exception.Message)" -Level Warn
    }
}
...
function Process-SyncQueue {
    ...
    foreach ($item in $queue) {
        ...
        try {
            ...
            if (-not (Test-Path $src)) {
                # Stale queue item: source no longer exists (e.g., job folder cleaned).
                # Mark as missing and do not retry.
                $item.status = "missing"
                $item.lastError = "Source missing: $src"
                $item.attemptCount = $MaxRetries
                Write-Log "Cloud sync skipped (stale queue item): $($item.lastError)" -Level Warn
                continue
            }
    ...
    # Prune stale items so they do not keep reappearing in logs
    $queue = @($queue | Where-Object { $_.status -ne "missing" })
    Save-SyncQueue -Queue $queue
...
function Start-EncryptionProcess {
    ...
    try {
        ...
        foreach ($item in $script:lstFiles.Items) {
            ...
            $result = Encrypt-FileCFEM -FilePath $filePath -Password $password -Iterations $iterations -OutputExtension $extension -OutputRoot $jobOutputRoot -SourceRoot $item.RootPath
        ...
        if ($script:chkSyncToCloud.IsChecked) {
            ...
            Write-GuiLog ("Cloud Sync Results - Copied: {0}  Failed: {1}  Pending: {2}" -f $syncSummary.Copied, $syncSummary.Failed, $syncSummary.Pending)
            Update-Status -Message ("Cloud sync complete (Copied: {0}, Failed: {1}, Pending: {2})" -f $syncSummary.Copied, $syncSummary.Failed, $syncSummary.Pending) -Detail ""
        }
        Update-Status -Message "Encryption complete: $successCount succeeded, $failCount failed"
        Write-GuiLog ("Encryption finished - Successful: {0}  Failed: {1}" -f $successCount, $failCount)
        Clear-FileSelection
    }
    catch {
...
function Start-FileMonitoring {
    try {
        ...
        $action = {
            ...
            try {
                $result = Encrypt-FileCFEM -FilePath $path -Password $script:sessionPassword `
...
try {
    ...
    $script:cmbProfiles = $window.FindName("cmbProfiles")
    $script:btnLoadProfile = $window.FindName("btnLoadProfile")
    $script:btnSaveProfile = $window.FindName("btnSaveProfile")
    $script:btnDeleteProfile = $window.FindName("btnDeleteProfile")
    $script:chkWatchEnabled = $window.FindName("chkWatchEnabled")
    $script:lstWatchFolders = $window.FindName("lstWatchFolders")
    $script:btnAddWatchFolder = $window.FindName("btnAddWatchFolder")
    $script:btnRemoveWatchFolder = $window.FindName("btnRemoveWatchFolder")
    $script:btnStartWatching = $window.FindName("btnStartWatching")
    $script:btnStopWatching = $window.FindName("btnStopWatching")
    ...
    $script:btnClearSyncQueue = $window.FindName("btnClearSyncQueue")
    ...
    if ($script:btnSaveProfile) {
        $script:btnSaveProfile.Add_Click({
            try {
                $name = ($script:cmbProfiles.Text).Trim()
                if ([string]::IsNullOrWhiteSpace($name)) {
                    Write-GuiLog "Profile: please enter a profile name."
                    Update-Status -Message "Profile not saved - enter a name." -Detail ""
                    return
                }
                $profiles = Load-Profiles
                $profiles[$name] = Get-CurrentSettingsSnapshot
                Save-Profiles $profiles
                Refresh-ProfilesUI
                $script:cmbProfiles.Text = $name
                Write-GuiLog "Profile saved: $name"
                Update-Status -Message "Profile saved: $name" -Detail ""
            } catch {
                Write-GuiLog ("Profile save failed: {0}" -f $_.Exception.Message)
            }
        })
    }

    if ($script:btnLoadProfile) {
        $script:btnLoadProfile.Add_Click({
            try {
                $name = ($script:cmbProfiles.Text).Trim()
                if ([string]::IsNullOrWhiteSpace($name)) { return }
                $profiles = Load-Profiles
                if (-not $profiles.ContainsKey($name)) {
                    Write-GuiLog "Profile not found: $name"
                    Update-Status -Message "Profile not found: $name" -Detail ""
                    return
                }
                Apply-SettingsSnapshot $profiles[$name]
                Write-GuiLog "Profile loaded: $name"
                Update-Status -Message "Profile loaded: $name" -Detail ""
            } catch {
                Write-GuiLog ("Profile load failed: {0}" -f $_.Exception.Message)
            }
        })
    }

    if ($script:btnDeleteProfile) {
        $script:btnDeleteProfile.Add_Click({
            try {
                $name = ($script:cmbProfiles.Text).Trim()
                if ([string]::IsNullOrWhiteSpace($name)) { return }
                $profiles = Load-Profiles
                if ($profiles.ContainsKey($name)) {
                    $profiles.Remove($name) | Out-Null
                    Save-Profiles $profiles
                    Refresh-ProfilesUI
                    $script:cmbProfiles.Text = ""
                    Write-GuiLog "Profile deleted: $name"
                    Update-Status -Message "Profile deleted: $name" -Detail ""
                }
            } catch {
                Write-GuiLog ("Profile delete failed: {0}" -f $_.Exception.Message)
            }
        })
    }

    # Watch folders UI
    if ($script:btnAddWatchFolder) {
        $script:btnAddWatchFolder.Add_Click({
            try {
                $dlg = New-Object System.Windows.Forms.FolderBrowserDialog
                $dlg.Description = "Select a folder to watch for new files"
                if ($dlg.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
                    $p = $dlg.SelectedPath
                    if ($p -and -not ($script:lstWatchFolders.Items -contains $p)) {
                        [void]$script:lstWatchFolders.Items.Add($p)
                        Write-GuiLog "Watch folder added: $p"
                    }
                }
            } catch {
                Write-GuiLog ("Add watch folder failed: {0}" -f $_.Exception.Message)
            }
        })
    }

    if ($script:btnRemoveWatchFolder) {
        $script:btnRemoveWatchFolder.Add_Click({
            try {
                $sel = $script:lstWatchFolders.SelectedItem
                if ($sel) {
                    [void]$script:lstWatchFolders.Items.Remove($sel)
                    Write-GuiLog "Watch folder removed: $sel"
                }
            } catch {
                Write-GuiLog ("Remove watch folder failed: {0}" -f $_.Exception.Message)
            }
        })
    }

    if ($script:btnStartWatching) { $script:btnStartWatching.Add_Click({ Start-Watching }) }
    if ($script:btnStopWatching) { $script:btnStopWatching.Add_Click({ Stop-Watching }) }
...
if ($script:btnClearSyncQueue) {
    $script:btnClearSyncQueue.Add_Click({
        try {
            Clear-SyncQueue
            # Update UI (best-effort)
            if (Get-Command -Name Write-GuiLog -ErrorAction SilentlyContinue) {
                Write-GuiLog "Sync queue cleared." "Info"
            } else {
                Write-Log "Sync queue cleared." -Level Info
            }
        } catch {
            Write-Log "Failed to clear sync queue: $($_.Exception.Message)" -Level Error
        }
    })
}
    ...
    Write-Log "$($script:AppName) v$($script:AppVersion) ($($script:BuildDate)) started"
    try { Write-Log ("Sync queue path: {0}" -f (Get-QueuePath)) -Level Info } catch { }
